...
def make_yaboot_header(target, target_dev):
    ...
    device = re.compile(r'/dev/[a-z]+')
    device_yaboot = os.popen4('ofpath %s' % device.search(target_dev).group())
    yaboot_conf.write(' \
    ...
    enablecdboot \
    ' % (boot_partition, device_yaboot, partition.search(target_dev).group(), target_dev, timeout) )
