...
class GtkTreeView(AutopilotGtkEmulatorBase):
    ...
    def _get_gail_treeview(self, ):
        ...
        # globalRect as us
        logger.debug('Selecting GtkTreeViewAccessible with same globalRect, or at least within close range')
    ...
    # being slightly different compared to it's GtkTreeView counterpart. i.e self
    def _get_all_atktreeviews_within_range(self, ):
        # lets get a root instance
        root = self.get_root_instance()
        assert root is not None
        treeviews = []
        # This is going to be slow! but what more can we do? we can't put an in range
        # arg on the select_many call :-(
        for tree in root.select_many('GtkTreeViewAccessible'):
            # There will be some tree's that don't have a globalRect property
            # no idea why but we can just ignore them as we can be sure this isn't
            # the one we want.
            ...
            in_range = True
            i = 0
            # Get a list of all ATK treeviews with globalRect within a 5px range of this GtkTreeView
            # FIXME: is 5px too much?? It's unlikely more than one tree will be within this range anyway,
            # as they would be overlaying each other. Even if they are only one will be visible (we hope!)
            for r in self.globalRect:
                if r not in range(tree.globalRect[i] - 5, tree.globalRect[i] + 5) :
                    in_range = False                    
                i += 1
            if in_range:
