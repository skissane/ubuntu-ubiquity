...
class GtkTreeView(AutopilotGtkEmulatorBase):
    ...
    def get_partition_table_dict(self, ):
        """ Returns a dict of named tuples generated by the list
            ...
            # we can now access any cell using Row number as Key and column name
             i.e table['Row Number'].ColumnName
             
            >>> item = table['Row2'].Mount
            
            We now have an introspectable object for that particular cell which we can use
        ...
        # therefore picking only the n items where n is the number of column names
